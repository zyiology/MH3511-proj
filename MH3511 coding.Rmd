...
JunToOctIndices = as.vector(sapply(6:10, function(i) seq(i,length(ensoIndex),12)))
ensoIndexCheck = rep(FALSE,length(ensoIndex)); ensoIndexCheck[JunToOctIndices] = TRUE

ensoPos = ensoIndex>=0 & ensoIndexCheck==TRUE
ensoNeg = ensoIndex<0 & ensoIndexCheck==TRUE
...
#p-value ~0.5, can be considered same variance?
t.test(ensoPos_sgSAT, ensoNeg_sgSAT,var.equal=TRUE)
```
...
months = c(1,2,10,11,12)
JunToOctIndices = unlist(sapply(months, function(i) seq(i,nrow(sat.timeshift.df),12)))

sat.timeshift.df.pruned = sat.timeshift.df[JunToOctIndices,]
n = nrow(sat.timeshift.df.pruned)
sat.timeshift.df.pruned = sat.timeshift.df.pruned[sample(n),]
...
numCores = detectCores()
cl = makeCluster(numCores)
clusterExport(cl,c('neuralnet','satFormula','sat.timeshift.df.pruned','groups','rsquarer','randomForest'))
results = parSapply(cl, 1:k, function(i) {
  
  train = sat.timeshift.df.pruned[groups!=i,]
  test = sat.timeshift.df.pruned[groups==i,]
