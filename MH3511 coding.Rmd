...
substemp.all <- substemp.all - 273.15

substemp.all.plot <- c(substemp.all) #561924 NA values

substemp.hist <- hist(substemp.all.plot,
                           main='Monthly Mean Sub Surf Temp in the Pacific (1982-2022)',
...
#calculate a baseline temperature for each month based on the reference period
substempRefStartIndex = 1; substempRefEndIndex = (2011-1982)*12
substemp.baselines = sapply(1:12,function(i) {
  monthValues = substemp.mean[seq(i,(2011-1982)*12,12)]
  return(mean(monthValues))
})

substemp.studyAnom = substemp.mean[(substempRefEndIndex+1):length(substemp.mean)] - rep(substemp.baselines,(2023-2011))
...
substemp.anom.hist <- hist(substemp.anom.vec,
                           main='Monthly Sub Surf Temp Anomaly in the Pacific (1982-2022)',
                           xlab = 'Temp Anomalies (deg C)')

substemp.anom.box <- boxplot(c(substemp.anom.vec),
                             main='Monthly Sub Surf Temp Anomaly in Pacific (1982-2022)',
